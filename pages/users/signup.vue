<template>
  <div class="flex justify-center">
    <div class="w-full md:w-1/2 lg:w-1/3 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
      <div class="mb-4">
        <FlashMessage :messages="errors" type="error" />
        <FlashMessage v-if="success" :messages="['User created in successfully!']" type="success" />
      </div>
      <form class="p-4 rounded-lg shadow-md bg-white">
...
const user = reactive<Omit<AuthUser, 'id'>>({
  name: "",
  age: null,
  phoneNumber: "",
  password: "",
...
const submitUser = async () => {
  try {
    const userResponse = await signup(user as AuthUser);
